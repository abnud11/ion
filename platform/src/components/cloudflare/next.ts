...
import { Function, FunctionArgs } from "../aws/function.js";
import {
...
export interface NextArgs extends SsrSiteArgs {
  ...
  /**
   * The command used internally to build your Nextjs app.
   *
  ...
  /**
   * Set a custom domain for your Nextjs app. Supports domains hosted either on
  ...
  /**
   * Set environment variables in your Nextjs app.
   *
   * :::tip
   * You can also `link` resources to your Nextjs app and access them in a type-safe way with the [SDK](/docs/reference/sdk/). We recommend linking since it's more secure.
   * :::
   *
   * @example
   * ```js
  ...
  /**
   * [Link resources](/docs/linking/) to your Nextjs app. This will:
   *
  ...
  /**
   * Path to the directory where your Next app is located.  This path is relative to your `sst.config.ts`.
   *
   * By default it assumes your Next app is in the root of your SST app.
   * @default `"."`
   *
   * @example
   *
   * If your Next app is in a package in your monorepo.
   *
   * ```js
   * {
   *   path: "packages/web"
...
/**
 * The `Next` component lets you deploy a [Next](https://nextjs.org/) app to AWS.
 *
 * @example
 *
 * #### Minimal example
 *
 * Deploy a Next app that's in the project root.
 *
 * ```js
 * new sst.cloudflare.Next("MyWeb");
 * ```
 *
 * #### Change the path
 *
 * Deploys the Next app in the `my-next-app/` directory.
 *
 * ```js {2}
 * new sst.cloudflare.Next("MyWeb", {
 *   path: "my-next-app/"
 ...
 * #### Add a custom domain
 *
 * Set a custom domain for your Next app.
 *
 * ```js {2}
 * new sst.cloudflare.Next("MyWeb", {
 *   domain: "my-next.com"
 ...
 * #### Link resources
 *
 * [Link resources](/docs/linking/) to your Next app. This will grant permissions
...
export class NextJs extends Component implements Link.Linkable {
  ...
  constructor(
    ...
    function buildPlan() {
      return all([openNextOutput, middlewareArgs]).apply(
        ([openNextOutput, middlewareArgs]) => {
          return validatePlan({
            server: middlewareArgs,
            assets: {
              assetsPrefix: "_assets",
              copy: openNextOutput.origins.s3.copy.filter(
                (copy) => copy.to !== "_cache",
              ),
            },
            routes: [
    ...
    function createServersLambda(): Output<Record<string, Function>> {
      return all([
        ...
        ([
          ...
          const serversOutput = Object.entries(openNextOutput.origins).filter(
            ([_, origin]) => origin.type === "function",
          ) as [string, OpenNextServerFunctionOrigin][];
    ...
    function createMiddleware(): Output<WorkerArgs> {
      return all([outputPath, openNextOutput]).apply(
        ([outputPath, openNextOutput]) => {
          const openNextOrigin = servers.apply(
            (
              origins,
            ): Record<
              string,
    ...
    function uploadCache() {
      return all([openNextOutput, outputPath]).apply(
        async ([openNextOutput, outputPath]) => {
          const bucketFiles: BucketFile[] = [];

          const copy = openNextOutput.origins.s3.copy.find(
            (copy) => copy.to === "_cache",
          );
          if (!copy) return;
